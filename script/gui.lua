...
local function create_buttons(player, surface_name)
    ...
    local function populate_flow(flow, button, direction)
        ...
        local input_ids = storage.factory_data.gui_inputs[direction]
        local dummy_id = 0
        for _, id in ipairs(input_ids) do
            if id == 0 then
                ...
                button.tags.num = id
                local item_name = storage.selected[surface_name][button.tags.direction][button.tags.num]
...
local function create_interface(player, surface_name)
    ...
    if screen["liteception-input-selector"] ~= nil then
        ...
        local outside_surface = game.get_surface(storage.factory[surface_name].placed_on_surface_name)
        local entity = outside_surface.find_entity(storage.factory[surface_name].name,
            storage.factory[surface_name].position)
        if entity == nil then
            return
        end
        ...
        player_storage.elements.main_frame[surface_name] = main_frame
        player.opened = main_frame

        create_buttons(player, surface_name)
    end
end
...
local function toggle_interface(player)
    ...
    local player_storage = storage.players[player.index]
    local main_frame = player_storage.elements.main_frame[surface_name]

    if main_frame == nil then
        create_interface(player, surface_name)
    else
        main_frame.destroy()
...
local function on_gui_elem_changed(event)
    ...
    local player = game.get_player(event.player_index)
    if player == nil then
        game.print("Error: player is nil.")
        return
    end
    local surface = player.surface
    local surface_name = surface.name:gsub("-factory%-floor$", "")
    ...
    inputs.set_input(element.tags.direction, element.tags.num, element.elem_value, surface_name)
    create_buttons(game.get_player(event.player_index), surface_name)
end
